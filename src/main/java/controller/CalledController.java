...
@WebServlet(urlPatterns = {"/calleds", "/called/form", 
		"/called/insert", "/called/delete", "/called/update"})
public class CalledController extends HttpServlet{
	...
	private void updateCalled(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			if (dao.update(called)) {
				ControllerUtil.sucessMessage(req, "Chamado '" + 
						called.getTitulo() + "' atualizada com sucesso.");
			}
			else {
				ControllerUtil.errorMessage(req, "Chamado '" + 
						called.getTitulo() + "' não pode ser atualizada.");
			}				
		} catch (ModelException e) {
			// log no servidor
			e.printStackTrace();
	...
	private void deleteCalled(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			if (dao.delete(called)) {
				ControllerUtil.sucessMessage(req, "Chamado '" + 
						called.getTitulo() + "' deletada com sucesso.");
			}
			else {
				ControllerUtil.errorMessage(req, "Chamado" + 
						called.getTitulo() + "' não pode ser deletado. "
	...
	private void insertCalled(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			if (dao.save(call)) {
				ControllerUtil.sucessMessage(req, "Chamado '" + call.getTitulo() 
				+ "' salva com sucesso.");
			}
			else {
				ControllerUtil.errorMessage(req, "Chamado '" + call.getTitulo()
				+ "' não pode ser salva.");
			}
		} catch (ModelException e) {
			// log no servidor
